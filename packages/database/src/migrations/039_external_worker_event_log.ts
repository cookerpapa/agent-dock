...
export async function up(db: Kysely<unknown>): Promise<void> {
  await db.schema
    ...
    .addColumn("last_projected_seq", "bigint")
    .execute();
  await sql`
    update session_event_cursors
       set last_projected_seq = last_persisted_seq
     where last_projected_seq is null
  `.execute(db);
  await db.schema
    .alterTable("session_event_cursors")
    .alterColumn("last_projected_seq", (column) => column.setNotNull())
    .execute();
  await sql`
    alter table session_event_cursors
      alter column last_projected_seq set default 0
  `.execute(db);
  ...
  await db.schema
    .alterTable("session_event_cursors")
    .addCheckConstraint(
      "session_event_cursors_projection_valid",
      sql`last_projected_seq >= 0 and last_projected_seq <= last_persisted_seq`,
    )
    .execute();
  await sql`
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  await db.schema
    .alterTable("session_event_cursors")
    .dropConstraint("session_event_cursors_projection_valid")
    .execute();
  await db.schema
    .alterTable("session_event_ids")
  ...
  await db.schema.alterTable("session_event_cursors").dropColumn("last_projected_seq").execute();
}
