...
function deferredDelay(cycle: number, requestedMs: number): number {
  const backoff = Math.min(5_000, 500 * 2 ** Math.min(cycle, 4));
  return Math.max(requestedMs, backoff);
}
...
export async function agentDockRunWorkflow(
  ...
  for (let cycle = 0; cycle < MAX_DISPATCH_CYCLES_PER_HISTORY; cycle += 1) {
    ...
    await sleep(deferredDelay(cycle, result.retryAfterMs));
  }
