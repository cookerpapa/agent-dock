...
export async function collectGitWorkspacePatch(
  ...
  // Intent-to-add records new paths without staging their contents. The normal
  // working-tree diff can then represent tracked edits, deletions, and newly
  // created files as one cumulative patch relative to the immutable baseline.
  // Apply it only to untracked paths: `git add -N --all` would stage tracked
  // deletions and make them disappear from the working-tree diff.
  const untracked = (
