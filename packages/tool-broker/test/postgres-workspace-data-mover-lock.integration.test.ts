...
import { PostgresWorkspaceDataMoverLock } from "../src/index.ts";
...
describe.skipIf(connectionString === undefined)("PostgreSQL Workspace Data Mover lock", () => {
  it("serializes one shared Volume across independent service connections", async () => {
    ...
    const first = new PostgresWorkspaceDataMoverLock(firstDatabase);
    const second = new PostgresWorkspaceDataMoverLock(secondDatabase);
    const events: string[] = [];
