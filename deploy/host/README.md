...
- registers `io.containerd.runsc.v1` with `platform = "kvm"` and the measured
  ...
  `software-gso = "false"`;
- labels the sandbox node and creates `RuntimeClass/agent-dock-gvisor`;
- creates restricted execution namespaces, default-deny network policies,
  no-authority workload identities, namespace-scoped Manager RBAC, and one
  resource-name-limited RuntimeClass read;
