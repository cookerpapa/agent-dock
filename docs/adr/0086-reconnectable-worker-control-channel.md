...
- Status: Accepted
- Date: 2026-08-08
- Refines: ADR-0031, ADR-0056 and ADR-0078

## Context

The Worker WebSocket is a management channel, not the production Run transport.
Temporal dispatches the exact-command Activity, and an executing Worker renews
its Session Lease directly in PostgreSQL. Nevertheless, the reconnecting client
previously revoked every active assignment as soon as its WebSocket to one
Control Plane process closed. A rolling restart or isolated Control Plane crash
therefore sacrificed otherwise healthy Agent Loops.
...
6. PostgreSQL-backed event ingestion remains independent of the management
   socket. A restarted browser/Control Plane resumes the committed stream using
...
- The Worker cannot accept new management commands while disconnected, but it
  can finish the exact Temporal Activity it already owns.
...
### Move Run execution back onto the WebSocket

That would reintroduce a second scheduler next to Temporal and reverse
ADR-0078.
