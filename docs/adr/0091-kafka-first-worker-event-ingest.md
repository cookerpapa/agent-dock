...
Worker local WAL
  ...
  -> one PostgreSQL transaction:
       replay rows + semantic projections + projected cursor + Kafka offset
  -> resumable SSE
```
...
3. The Kafka projector stores its consumed partition offset in the same
   PostgreSQL transaction as replay rows, semantic projections and
   `last_projected_seq`. It commits Kafka's group offset only after that
...
6. Local/self-hosted mode keeps the simpler direct PostgreSQL implementation
   and does not require Kafka or the internal ingest endpoint.
...
- Projector crashes after PostgreSQL commit but before Kafka offset commit: the
  record is redelivered and the PostgreSQL transaction-local offset makes the
  replay idempotent.
...
- `worker_event_outbox` and its raw payload copy are removed from the current
  schema.
- PostgreSQL remains the authority for product state and the browser replay
  projection, but not a second raw Worker-event transport log.
