...
- Status: accepted, amended for the PostgreSQL/Pi SDK worker pool
- Date: 2026-07-19

## Context

Each trusted Pi Worker keeps one authenticated outbound WebSocket to the Control
Plane for registration, capacity, heartbeats and active steer. Workers claim
Runs directly from PostgreSQL and publish live events through the Event
Gateway; the WebSocket is not an execution queue or event durability boundary.
...
5. Durable events already accepted into the Worker WAL continue through the
   Event Gateway and are de-duplicated by `(run, attempt, sequence)`; they do
   not wait for the management socket.
6. Reconnect does not reassign a Run. PostgreSQL RunAttempt lease/fence checks
   determine whether the current Worker may continue or a replacement may
   claim later work.
7. A Tool execution whose outcome is ambiguous is never blindly replayed. It is
   failed or reconciled under the recoverable Tool protocol in ADR-0080.
