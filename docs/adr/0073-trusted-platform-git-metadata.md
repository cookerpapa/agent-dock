...
PiCloud creates a synthetic root Git repository for every Workspace. It is
...
can access `.pi-cloud-runtime/git`.
...
4. the Volume Gateway invokes Git with explicit trusted paths:
   ...
   ```text
   GIT_DIR=<envelope>/.pi-cloud-runtime/git
...
- Root `git status` no longer discovers PiCloud's synthetic repository.
  Coding Tools use PiCloud's Patch result for platform review, while user
