...
| `AGENT_DOCK_KUBECTL_BIN` | `/usr/local/bin/k3s` in the local Cube installer | Optional native Kubernetes client override. When omitted, the installer uses `k3s kubectl` and does not trust a WSL/Windows PATH wrapper. | Used only while installing/reconciling the local Cube plane. |
