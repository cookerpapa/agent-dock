...
PiCloud must grow from the current bounded single logical execution plane to
...
Kafka is not adopted merely from a projected capacity number. PiCloud first
...
an PiCloud-owned `DurableEventLog` port so storage semantics remain testable
...
client is isolated behind the PiCloud `DurableEventLog` adapter. PostgreSQL
...
PiCloud must not recreate that scheduler. PiCloud's Sandbox Manager is a
...
multi-node acceptance suite before an PiCloud release claims production HA.
...
PiCloud builds only the bounded glue that no adopted component owns:
...
PiCloud does not build a workflow engine, distributed log, KVM scheduler,
