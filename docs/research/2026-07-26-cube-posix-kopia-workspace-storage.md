...
Can PiCloud replace node-local Cube snapshot authority with Cube's supported
...
PiCloud therefore needs a bounded pre-mounted POSIX plugin, not a second
...
Kopia's repository server access controls are not used as PiCloud's tenant
security boundary: official documentation notes that clients that know object
IDs can read repository objects. PiCloud instead keeps all repository
...
2. a narrow authenticated data-mover API that validates PiCloud identity and
