...
- S3/MinIO checkpoints and immutable repository seeds;
...
Untrusted browser input / repository / model output
       ...
       authenticated commands/events
                 v
   Trusted Pi Runner + Model Gateway
                 |
...
The repository importer receives no GitHub token. It can fetch only a normalized
public `owner/repository` at an exact commit through its dedicated egress
network. Private repository credentials and write-back require a future trusted
GitHub Gateway threat model.
