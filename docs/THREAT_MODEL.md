...
| Cube snapshot reference is stolen, tampered with, or rebound by an old Attempt | reference binds tenant/Workspace/image/environment/source binding and fence; recovery authority is AES-256-GCM sealed with that identity as AAD; restore requires a strictly higher fence and rotates activation/binding/secret before Tool execution | tamper/binding/fence Provider tests and live source-destroy/cold-restore gate |
| Historical snapshot preview becomes a hidden write/exec channel | distinct read-only service token; sealed snapshot clone is never rebound; one indexed regular file is opened beneath `/workspace`, hash-checked, destroyed and confirmed absent before capacity release | credential-separation RPC test, traversal/hash Provider tests and live materializer gate |
| Incomplete snapshot mark set deletes retained Workspace data | Control Plane verifies every retained MinIO object before a GC-only request; missing/corrupt data, protocol overflow, missing Cube references or ambiguous ownership abort the scan; deletion also requires durable two-pass observation and a 24-hour grace | reference scanner, GC state/restart and credential-separation tests |
...
| Cube snapshot storage node/disk is lost | current private single-node profile fails closed and makes no off-host recovery claim; multi-node/replicated snapshot data or a portable Kopia/REAPI data mover is a release gate | ADR-0064 limitation and future node-loss drill |
...
| Compose backup is tampered with, partially restored, or overwrites live state | AES-GCM authenticated payload, scrypt key derivation, per-authority hashes, safe archive paths, exact image IDs, new empty project/runtime only; Cube-native snapshot bytes require a separate coordinated Cube storage backup | crypto tamper/wrong-key check, Compose restore drill and explicit ADR-0064 boundary |
